...
const PostCardHeader = ({ userName, userAvatar, speciesName, postId, postUserId, onEdit, onDelete, onInfo, isOverlay = false }: PostCardHeaderProps) => {
  const { user } = useAuth();
  const isOwner = user?.id === postUserId;

  // If overlay mode, only show the menu button
  if (isOverlay) {
    return (isOwner || onInfo) ? (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button 
            variant="ghost" 
            size="icon" 
            className="w-9 h-9 rounded-full bg-white/20 backdrop-blur-sm hover:bg-white/30 transition-all"
