...
const CollectionsPage = () => {
  ...
  const filters = [{
    ...
    id: "birds",
    label: "Birds",
    count: savedPosts.filter(p => p.category === "bird").length
  }, {
    id: "mammals",
    label: "Mammals",
    count: savedPosts.filter(p => p.category === "mammal").length
  }, {
    id: "plants",
    label: "Plants",
    count: savedPosts.filter(p => p.category === "plant").length
  }, {
    id: "insects",
    label: "Insects",
    count: savedPosts.filter(p => p.category === "insect").length
  }];
  useEffect(() => {
    fetchSavedPosts();
  }, []);
  const fetchSavedPosts = async () => {
    ...
    try {
      ...
      const formattedPosts = data?.map(post => ({
        ...
        category: 'bird',
        // Default category, in real app this would come from AI identification
  ...
  const filteredPosts = activeFilter === "all" ? savedPosts : savedPosts.filter(post => post.category === activeFilter.slice(0, -1)); // Remove 's' from 'birds' etc
  ...
  return <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50">
      ...
      <div className="max-w-md mx-auto px-4 py-4">
        <div className="flex items-center space-x-2 overflow-x-auto pb-2">
          {filters.map(filter => <Button key={filter.id} variant={activeFilter === filter.id ? "default" : "outline"} size="sm" onClick={() => setActiveFilter(filter.id)} className="whitespace-nowrap text-slate-700 text-sm bg-green-600 hover:bg-green-500 rounded">
      ...
      <div className="max-w-md mx-auto px-4 pb-24">
        {filteredPosts.length === 0 ? <div className="text-center py-12">
          ...
          </div> : <div className={viewMode === "grid" ? "grid grid-cols-2 gap-4" : "space-y-4"}>
            {filteredPosts.map(post => <div key={post.id} className={viewMode === "grid" ? "break-inside-avoid" : ""}>
                {viewMode === "list" ? <PostCard post={post} onLike={handleLike} onSave={handleSave} onComment={() => {}} onShare={() => {}} onEdit={handleEdit} onDelete={handleDelete} onInfo={handleInfo} /> : <div className="glass-card rounded-xl overflow-hidden">
